...
watchcat_run_script() {
	logger -p daemon.info -t "watchcat[$$]" "Running script \"$1\" for state \"$2\"."
	"$1" "$2"
}
...
watchcat_monitor_network() {
	...
	script="$9"
        ...
        failure_period_default="$failure_period"
        ping_frequency_interval_default="$ping_frequency_interval"
	...
	while true; do
		...
		for host in $ping_hosts; do
			...
			if [ "$ping_result" -eq 0 ]; then
				time_lastcheck_withinternet="$time_now"
                                if [ "$failure_period" -gt "$failure_period_default" ] || [ "$ping_frequency_interval" -gt "$ping_frequency_interval_default" ]; then
                                        failure_period="$failure_period_default"
                                        ping_frequency_interval="$ping_frequency_interval_default"
                                        logger -p daemon.info -t "watchcat[$$]" "Back $host so back to default values ping_frequency_interval_default $ping_frequency_interval_default seconds and failure_period_default $failure_period seconds"
                                        if [ "$script" != "" ]; then
                                            logger -p daemon.info -t "watchcat[$$]" "execute script \"$script\" \"up\""
                                            watchcat_run_script "$script" "up"
                                        fi
                                fi
			else
				if [ "$script" != "" ]; then
					logger -p daemon.info -t "watchcat[$$]" "Could not reach $host for \"$((time_now - time_lastcheck_withinternet))\" seconds. Running script after reaching \"$failure_period\" seconds"
				elif [ "$iface" != "" ]; then
		...
		[ "$((time_now - time_lastcheck_withinternet))" -ge "$failure_period" ] && {
			if [ "$script" != "" ]; then
				watchcat_run_script "$script" "down"
			else
			...
			# Restart timer cycle.
			time_lastcheck_withinternet="$time_now"
                        failure_period="$((failure_period + failure_period))"
                        if [ "$failure_period" -gt "86400" ]; then
                                failure_period="86400"
                        fi
                        ping_frequency_interval="$((ping_frequency_interval + ping_frequency_interval))"
                        if [ "$ping_frequency_interval" -gt "3600" ]; then
                                ping_frequency_interval="3600"
                        fi
		}
